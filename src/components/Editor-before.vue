  ...
  mounted() {
    ...
    setTimeout(() => {
      if (this.$refs.editor && this.$refs.editor.getQuill()) {
        ...
        //this.adjustEditorHeight(); // 调整编辑器高度
      }
    }, 0); 
  },
  methods: {
    ...
    //   if (editor) {
    //     const editorContent = editor.$el.querySelector('.ql-editor');
    //     editorContent.style.height = 'auto'; // 先重置高度
    //     editorContent.style.height = editorContent.scrollHeight + 'px'; // 设置为内容的高度
    //   }
    // }
  },
};
...
<template>
  <div class="editor-container">
    ...
    <input type="text" v-model="author" placeholder="请输入作者名称" class="author-input" />
        <!-- <select id="event" v-model="event" required>
